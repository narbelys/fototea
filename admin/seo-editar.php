...
if($_POST){
	...
	$values = "page='".$_POST['page']."',title = '".$_POST['title']."', description = '".$_POST['description']."',keywords = '".$_POST['keyword']."'";
	$glosarioIn = updateTable("seo",$values,"id = $_GET[i]");
	if($glosarioIn != false){
	
	?>
<script>
	alert("Se ha modificado la pagina correctamente.");
	window.location="seo.php";
	</script>
    <?
	}else{ ?>
    <script>
	alert("No se ha  podido modificar la pagina correctamente.");
	window.history.back();
	</script>
    <?
	}
...
$seo = listAll("seo","WHERE id = $_GET[i]");
                ...
                <div class="content">
                    ...
                    <form id="form1" name="form1" class="stdform" method="post" action="" enctype="multipart/form-data">
                    	<p>
                            ...
                            <span class="field"><input type="text" name="page" id="page" class="longinput" value="<?=$rs_seo->page;?>"/></span>
                        </p>
                        <p>
                        	<label>Titulo:</label>
                            <span class="field"><input type="text" name="title" id="title" class="longinput"  maxlength="70" value="<?=$rs_seo->title;?>"/></span>
                        </p>
                        <p>
                        	<label>Descripci&oacute;n:</label>
                            <span class="field"><input type="text" name="description" id="description" class="longinput"  maxlength="150" value="<?=$rs_seo->description;?>"/></span>
                        </p>
                        <p>
                        	<label>Keywords:</label>
                            <span class="field"><input type="text" name="keyword" id="keyword" class="longinput"  maxlength="120" value="<?=$rs_seo->keywords;?>"/></span>
                        </p>
